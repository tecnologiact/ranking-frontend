...
export default function ProcessoPage({ params }) {
  ...
  async function handleUpload(file) {
    setUploading(true);
    setUploadProgress(null); // indeterminado enquanto lê o arquivo no navegador
  ...
  // Exporta a planilha em foco (a aba ativa se for um resultado gerado pela IA;
  // senão, o último resultado gerado). Download explícito do analista.
  async function handleExportarAtivo() {
    let doc = docs.find((d) => d.id === activeDocId);
    if (!doc || doc.kind !== "gerado" || !doc.downloadAcao) {
      doc = [...docs].reverse().find((d) => d.kind === "gerado" && d.downloadAcao);
    }
    if (!doc) {
      addToast(
        "Peça à IA no chat para gerar o resultado (ranking ou planilha) antes de exportar.",
        "info"
      );
      return;
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 150px)", minHeight: 520 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, flexShrink: 0 }}>
        ...
        <button onClick={handleExportarAtivo} style={pill("#EE222B", "#fff")}>
          Exportar planilha
        </button>
      </div>
